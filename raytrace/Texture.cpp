...
float4 Texture::sample_linear(const float3& texcoord) const
{
  ...
  int i1 = (U%width) + (V%height) * width;
  int i2 = ((U+1)%width) + (V%height) * width;
  int i3 = (U%width) + ((V+1)%height) * width;
  int i4 = ((U+1)%width) + ((V+1)%height) * width;

  return bilerp(fdata[i1], fdata[i2], fdata[i3], fdata[i4], c1, c2);
