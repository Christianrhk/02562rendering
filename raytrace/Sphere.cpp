...
bool Sphere::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
  ...
  float3 org_center = (r.origin - center);
  float bhalf = dot(org_center,r.direction);
  float c = dot(org_center,org_center)-(radius*radius);

  if(bhalf*bhalf-c<0)
    return false;

  float discriminant = sqrt(bhalf*bhalf-c);
  float t1 = -bhalf - discriminant;
  float t2 = -bhalf + discriminant;
  ...
  float3 n = normalize(hit.position - center);
