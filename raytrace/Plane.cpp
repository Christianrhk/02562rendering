...
bool Plane::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
  ...
  float denominator = dot(r.direction,get_normal());
  if(fabs(denominator) > 0.0001f){ // Check for division by 0
    float t = -(dot(r.origin, get_normal()) + d) / denominator;
...
{ 
  ...
  float3 x =  hit_pos - position;
  u = tex_scale * dot(x, onb.m_tangent)/length(onb.m_tangent);
  v = tex_scale * dot(x, onb.m_binormal)/length(onb.m_binormal);
}